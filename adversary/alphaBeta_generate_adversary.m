function  [nodenext,GraphUpadte,BestValue,done]=alphaBeta_generate_adversary(Count,GraphName,End_Generation,Max_Children, Min_Children,c1,c2,c3,c4,c5,u)%genearate the next node by deepth first
    ...
    if GraphUpadte.Nodes.Finish(Count)==0 %The node is not finish, Countinue genearte the next part of the node;
         ...
         GraphUpadte.Nodes.rid(Count+1)=0;
           ...
           if  GraphUpadte.Nodes.Law(parent)==0%Min_Children=4;
               ...
               GraphUpadte.Nodes.Y(Count+1) =  {UpdateY_adversary(table2array(GraphUpadte.Nodes.Y(parent)),GraphUpadte.Nodes.Children(parent),u,c1,c2,c3,c4,c5)};                                                        
               GraphUpadte.Nodes.Cov(Count+1) = {kalmanRiccatiCov_adversary(table2array(GraphUpadte.Nodes.Position(parent)),table2array(GraphUpadte.Nodes.Y(Count+1)),table2array(GraphUpadte.Nodes.Cov(parent)),GraphUpadte.Nodes.Children(parent),c1,c2,c3,c4,c5)};         
           ...
           while GraphUpadte.Nodes.Finish(parent)%to go to the upper part to find the node that is not finished. if all done break;
               ...
               if  ~mod(GraphUpadte.Nodes.Generation(parent),2) && parent ~= 1 % parent node is to choose the control law level?return the worst measurement
...
%  %beta pruning                
           ...
           if  GraphUpadte.Nodes.Children(1) > Min_Children
                    done=1;
                    return
%                    error('It is done!!')
