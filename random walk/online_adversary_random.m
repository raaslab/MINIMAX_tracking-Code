function [X_after,Y_after,Cov_after] = online_adversary_random(X_before,Y_before,Cov_before)


c1=eye(2);
c2=[1.2 0.05;0.15 1.05];
...
[Count,DF,AAA,done]=alphaBeta_generate_random(Count,DF,End_Generation,Max_Children, Min_Children,c1,c2,c3,c4,c5,u);
bb=bb+1;
end
...
for j=1:Count
 ...
 k=0;
 for i=2:bb-1  
    if DF.Nodes.Parent(i)==j
       k=k+1; 
       R(k)=i;
    end
 end
 ture_target=R(unidrnd(k));
 
% 
Cov_after=cell2mat(DF.Nodes.Cov(k));
X_after=cell2mat(DF.Nodes.Position(j));
Y_after=cell2mat(DF.Nodes.Y(ture_target));
end
